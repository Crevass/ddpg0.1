	...
	for episode in range(MAX_EPISODE):
		...
		while True:
			...
			if (memory.get_size() > HORIZON):
				...
				if (write_pointer % 50 == 0):
					summary = sess.run(merged, feed_dict={agent.s1: bs, agent.actor1: ba, agent.r:br, agent.s2: bs_, agent.terminate: be})
					writer.add_summary(summary, write_pointer)
				agent.train1(bs, ba, br, bs_, be)
			s = s_
			ep_reward += r
			step_pointer += 1
			if done:
